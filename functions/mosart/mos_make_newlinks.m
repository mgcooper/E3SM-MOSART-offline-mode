function [newlinks,inlet_ID,outlet_ID] = mos_make_newlinks(links,nodes)
    ...
    inlet_ID        = [];       % initialize the inlet and outlet ID arrays
    outlet_ID       = [];
...
for n = 1:length(links)
    ...
    if length(us_conn_id) == 1
        inlet_ID    = [inlet_ID;links(n).id];
    end
    
    if length(ds_conn_id) == 1
        outlet_ID   = [outlet_ID;links(n).id];
    end
end
