...
for n = 1:length(slopes)
   ...
   if isfield(slopes,'BoundingBox')
      ...
      if islatlon(yb(1),xb(1))
         ...
         % we're done, could assign the data and continue, but instead check if
         % the bbox can be computed in lat/lon from the provided data
         ...
         if isfield(slopes,'Lat_hs')
            ...
            %------------------------------------------------------------------
            % BELOW HERE NOT SURE WHY I CONVERTED TO X/Y FIRST TO GET BBOX THEN
            % WENT BACK TO LAT/LON ... SHOULD BE ABLE TO JUST USE THE LAT/LON
            % ... the issue is if the data doesn't have lat/lon, then I need to
            % know the projection and convert to lat/lon.
            ...
            % % check if the hillslope boundary X_hs/Y_hs are actually lat/lon
            % tf = islatlon(y(1),x(1));

            % % convert to xy in order to use boundingbox (can I just use min/max?)
            % if tf
            %    [x,y,f] = ll2utm([y,x]);
            % end
...
%    x = slopes(n).X_hs;
% 
%    % if the hillslope boundary X_hs/Y_hs are actually lat/lon, convert to x,y
%    tf = islatlon(y(1),x(1));
%    if tf
%       [x,y,f] = ll2utm([y,x]);
%    end
...
%    end
% 
%    % looks like I commented this out b/c new data has Lat/Lon so I can just use
%    % geoquadpt directly, meaning steps above aren't necessary
