function CU = remapWaterDemand(CUTW,W,IN,gcam_weights,idx_gcam)
   ...
   for m = 1:size(W,1)
      ...
      % use for the subbasin(s) that this mesh cell is in/on CUTW(:,IN{m}),
      % scaled by the ratio of the mesh cell area to the subbasin(s) area W{m},
      % and by the ratio of monthly GCAM demand to annual GCAM demand WT'. Some
      % identities: sum(IN{m}) == numel(W{m}), numel(CUTW(:,IN{m}) * W{m}) ==
      % size(CUTW,1)
      cu = CUTW(:,IN{m}) * W{m} * WT' ;
   ...
   % hold on; scatter(GCAM_X,GCAM_Y,100,'r','filled')
   % scatter(GCAM_X(igcam(m)),GCAM_Y(igcam(m)),100,'g','filled')
   % scatter(XC(m),YC(m),80,'y','filled')
   %
   % test = reshape(cu.',[],1); figure; plot(Time,test); ylabel('Monthly
   % Consumptive Use (m3/s)')
end
