function [DependentCells,iDependentCells] = makeDamDependency(...
   ...
   %dam in xydams. each row of the output array 'DependentCells' contains the
   %IDs of the depenent cells for the dam corresponding to that row.
   %
   ...
   %  cells inside a region of interest. cells outside the mask are not included
   %  in the DependentCells.
   %
   ...
   addRequired(parser, 'ID', @isnumeric);
   addRequired(parser, 'dnID', @isnumeric);
   addRequired(parser, 'xydams', @isnumeric);
   addRequired(parser, 'xyzmesh', @isnumeric);
   addOptional(parser, 'searchradius', 10000, @isnumeric);
   addParameter(parser, 'userID', nan, @isnumeric);
   addParameter(parser, 'iflowline', false, @islogical);
   addParameter(parser, 'mask', [nan nan], @isnumeric);
   parse(parser,ID,dnID,xydams,xyzmesh,varargin{:});

   searchrad = parser.Results.searchradius;
   iflowline = parser.Results.iflowline;
   userID = parser.Results.userID;
   xymask = parser.Results.mask;

   % useflowline determines whether we start the search for dependent cells at
   % the grid cell that contains each dam (useflowline=false), or the grid cell
   % nearest each dam that contains a flowline (useflowline=true). if
   % useflowline is true, then iflowline must be provided, where iflowline is a
   % logical array of size equal to xmesh/ymesh that is true for cells that
   % contain a flowline
   ...
   % % convert the mask to a polygon if it comes in as a shapefile if
   % ~isa(mask,'polyshape')
   ...
   % optionally exclude mesh cells outside of the provided polygon mask
   for n = 1:numdams
      ...
      % query the mesh kdtree to find all downstream cells within rxy distance
      % of the flowline. all cells in xyquery are below the dam, but the
      % rangesearch will return cells that are above the dam, they get trimmed
      % next. inearby is a cell array with one cell per downstream point in
      % xyquery. each cell has the mesh indices within rxy distance of the
      % xyquery vertex.
      inearby = rangesearch(meshTree,xyquery,searchrad);

      % concatenate and remove cells that are above zdams(n) (the elevation of
      % the mesh cell that contains the dam)
   ...
   % to use cellfun, need to add the unique(...) step idepends{n} =
   % rangesearch(meshTree,xymesh(idownstream{n},:),searchrad).';
