function [Line,Mesh] = findCellsOnVectorFlowline(Line,Mesh)
   ...
   % Syntax:
   %
   %  [Line,Mesh] = findCellsOnVectorFlowline(Line,Mesh) finds mesh cells that
   %  are "on" i.e. that contain a flowline vertex and adds the indices to the
   %  Line and Mesh geotables.
   %
   % Author: Matt Cooper, 05-Oct-2022, https://github.com/mgcooper

   % input parsing
   parser = inputParser;
   parser.FunctionName = 'findCellsOnVectorFlowline';
   addRequired(parser, 'Mesh', @isstruct);
   addRequired(parser, 'Line', @isstruct);
   parse(parser,Mesh,Line);

   % each line is comprised of segments with vertices that are near but not
   % exactly equal to the mesh cell centroids. if they were exactly equal we
   % could use == to find the mesh cells that contain each segment. instead,
   % iterate over the  vertices of each line segment and find the nearest mesh
   % cell, and add that index to the line attributes.
   ...
   % make a figure to see the mesh if desired figure('Position', [50 60 1200
   % 1200]); hold on; patch_hexmesh(Mesh); hold on;
   ...
   for n = 1:numel(Line)
      ...
      % this can be used to see the flowline vertices geoshow(Line(n));
   end
