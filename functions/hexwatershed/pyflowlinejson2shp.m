function S = pyflowlinejson2shp(geojsonfile,jsoninfofile,varargin)
   ...
   addRequired(parser, 'geojsonfile', @(x)ischar(x));
   addRequired(parser, 'jsoninfofile', @(x)ischar(x));
   addParameter(parser, 'savefile', false, @(x)islogical(x));
   addParameter(parser, 'filename', 'tmp.shp', @(x)ischar(x));
   parse(parser,geojsonfile,jsoninfofile,varargin{:});

   geofile  = parser.Results.geojsonfile;
   jsonfile = parser.Results.jsoninfofile;
