...
for n = 1:length(us_link)
   idxn = ismember([newlinks.link_ID],us_link(n));
   idxi = ismember([newlinks(idxn).ds_conn_ID],rm_link);

   % make the replacements
   newlinks(idxn).ds_link_ID = rp_link;
   newlinks(idxn).ds_node_ID = rp_node;
   newlinks(idxn).ds_conn_ID(idxi) = rp_link;
end

% deal with links that merge with rm_link at its downstream node
for n = 1:length(ds_link)
   idxn = ismember([newlinks.link_ID],ds_link(n));
   idxi = ismember([newlinks(idxn).ds_conn_ID],rm_link);
   if slope_flag == true
      idxia = ismember([newlinks(idxn).ds_hs_ID],rm_slope);
      idxib = ismember([newlinks(idxn).ds_hs_ID],-rm_slope);
   else
      idxia = [];
      idxib = [];
   end
   newlinks(idxn).ds_hs_ID(idxia) = [];      % remove entirely
   newlinks(idxn).ds_hs_ID(idxib) = [];      % remove entirely
   newlinks(idxn).ds_conn_ID(idxi) = [];     % remove entirely
end
