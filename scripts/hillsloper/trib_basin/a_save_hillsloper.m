...
pathdata = '/Users/coop558/work/data/interface/hillsloper/trib_basin';
pathsave = '/Users/coop558/work/data/interface/hillsloper/trib_basin/newslopes';
...
fname = fullfile(pathdata,'Sag_gage_HUC_filled.tif');
DEM = GRIDobj(fname);
...
for n = 1:length(slopes)
   ...
   if strcmp(R.CoordinateSystemType,'geographic')
      hslp = round(mean(geointerp(G.Z,R,yhs(ihs),xhs(ihs),'linear'),'omitnan'),4);
      ...
      hslp = round(mean(mapinterp(G.Z,R,xhs(ihs),yhs(ihs),'linear'),'omitnan'),4);
      harea = round(area(hspoly),0);
   end
