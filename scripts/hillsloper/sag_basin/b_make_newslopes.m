...
% pick back up on the other version, it has the right order of operations, but
% this one needs to be preserved b/c it has the rm_links for the v3 sag

opts = const('save_data',false,'plot_map',true,'plot_slopes',false);
...
% the only difference b/w these two is that makenewslopes adds +1 to link_ID
% [newlinks,newslopes,info] = makenewlinks(slopes,links,nodes,false);
[newslopes,newlinks,info] = makenewslopes(slopes,links,nodes,opts.plot_slopes);

labelfields = {'links','id','slopes','link_id'};
plothillsloper(slopes,links,nodes,'pauseflag',true,'labelfields',labelfields);

% % isequal(fieldnames(testslopes),fieldnames(newslopes))
% % fields = fieldnames(testslopes);
% % for n = 1:numel(fields)
% %    dat1 = [testslopes.(fields{n})];
% %    dat2 = [newslopes.(fields{n})];
...
% % newID = [newslopes.ds_link_ID];
% % find(testID~=newID)
% % figure; plot([testslopes.link_ID],[newslopes.link_ID],'o'); addOnetoOne;
% 
% 
% 
...
%    rm_flag  );
% 
% % repeat for links with duplicate hillslopes, meaning rm_flag == false
% rm_link = 1579;
% rp_link = 2477;
...
% 
% % merge the slopes
% newslopes = removeslopes(newslopes,rm_slope,rp_slope,rp_flag);
% 
...
% 
% for n = 1:length(IDs)
%    newlinks(n).link_ID = IDs(n);
%    newlinks(n).ds_link_ID = dnIDs(n);
% end
% 
...
%    slope_info = mergeslopes(slopes,slope_id); % was slopes, changed to newslopes
% 
...
% mosartslopes = mos; links = newlinks; slopes = newslopes; clear mos;
% 
