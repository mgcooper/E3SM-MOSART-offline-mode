...
load(fullfile(pathdata, 'mosart_hillslopes'));
...
rmfields = {'hs_ID','us_hs_ID','ds_hs_ID','us_conn_ID','ds_conn_ID', ...
   'us_node_ID','ds_node_ID','us_link_ID','waterbod_1','waterbody_',};
...
linksgeo = rmfield(linksgeo,{'X','Y','link_X','link_Y'});
linksmap = rmfield(linksmap,{'Lat','Lon','link_Lat','link_Lon'});

%% SLOPES

% Feb 2024 - commented this out, I needed to make the merged links shapefile for
%  new full sag but did not have time to finish updating the field names here
...
% Replaced the loop with these, but kept the loop in case I want to add the ID
[xmap, ymap] = geostructCoordinates(linksmap, 'projected');
[ygeo, xgeo] = geostructCoordinates(linksgeo);
...
if opts.save_shp == true
   ...
   %
   writeGeoShapefile(linksgeo,fullfile(pathdata,'links_mosart_geo.shp'));
   shapewrite(linksmap,fullfile(pathdata,'links_mosart_tmp_aka.shp'));

   % This took a very long time to run for the latest Sag -v2. I think the only
   % reason I use writeGeoShapefile is b/c it adds the projection so I don't
   % need the _tmp.
   % writeGeoShapefile(linegeo,fullfile(pathdata,'streams_mosart_geo.shp'));

   shapewrite(linegeo,fullfile(pathdata,'streams_mosart_geo.shp'));
   shapewrite(linemap,fullfile(pathdata,'streams_mosart_tmp_aka.shp'));

   % writeGeoShapefile(slopesgeo,fullfile(pathdata,'slopes_mosart_geo.shp'));
   % shapewrite(slopesmap,fullfile(pathdata,'slopes_mosart_tmp_aka.shp'));
   % shapewrite(linksmap,fullfile(pathdata,'links_mosart_tmp_aka.shp'));
   % shapewrite(linemap,fullfile(pathdata,'streams_mosart_tmp_aka.shp'));
end
...
%    y = [y;nan;slopes(n).Y_link']; %#ok<*AGROW>
